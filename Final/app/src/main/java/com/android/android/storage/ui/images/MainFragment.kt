...
import com.android.android.storage.*
import com.android.android.storage.databinding.FragmentMainBinding
...
class MainFragment : Fragment(), ActionMode.Callback {
    ...
    when (action) {
      ...
      is MainAction.FavoriteChanged -> {
        ...
        if (action.favorites.isEmpty()) {
          Snackbar.make(binding.root, R.string.no_favorite_media, Snackbar.LENGTH_SHORT).show()
        }
      }
      is MainAction.TrashedChanged -> {
        ...
        if (action.trashed.isEmpty()) {
          Snackbar.make(binding.root, R.string.no_trashed_media, Snackbar.LENGTH_SHORT).show()
        }
      }
  ...
  override fun onPrepareActionMode(mode: ActionMode?, menu: Menu?): Boolean {
    ...
    if (media.favorite) {
      menu?.findItem(R.id.action_favorite)?.title = getString(R.string.action_favorite_remove)
    } else {
      menu?.findItem(R.id.action_favorite)?.title = getString(R.string.action_favorite_add)
    }

    if (media.trashed) {
      menu?.findItem(R.id.action_trash)?.title = getString(R.string.action_trash_remove)
    } else {
      menu?.findItem(R.id.action_trash)?.title = getString(R.string.action_trash_add)
    }
  ...
  private fun loadFavorites() {
    if (!hasSdkHigherThan(Build.VERSION_CODES.Q)) {
      Snackbar.make(binding.root, R.string.not_available_feature, Snackbar.LENGTH_SHORT).show()
      return
    }
